...
-- The later discount migrations replaced these functions without the fields
-- introduced by partial production-order receipts. Restore one compatible
-- JSON contract and save path without rewriting already-applied migrations.
...
begin
  ...
  if pg_catalog.jsonb_typeof(command -> 'orderDate') is distinct from 'string'
    ...
    or (command ? 'discount' and pg_catalog.jsonb_typeof(command -> 'discount') is distinct from 'number')
  ...
  if pg_catalog.to_char(order_date_value, 'YYYY-MM-DD') <> command ->> 'orderDate'
    ...
    or expected_date_value < order_date_value
    or discount_value < 0
    or discount_value > 9999999999.99
    or discount_value <> pg_catalog.round(discount_value, 2) then
    raise exception using errcode = 'P0001', message = 'INVALID_PRODUCTION_ORDER';
  end if;
  ...
  if creating then
    ...
    insert into public.production_orders (
      client_request_id, order_number, order_date, expected_date, note, discount
    ) values (
      request_id,
      'PO-' || pg_catalog.to_char(order_date_value, 'YYYYMMDD') || '-' || pg_catalog.lpad(next_order_sequence::text, 6, '0'),
      order_date_value, expected_date_value, note_value, discount_value
  ...
  for line in select value from pg_catalog.jsonb_array_elements(command -> 'lines') loop
    ...
    if not creating then
      retained_received_quantity := coalesce((retained_received_by_variant ->> line_variant_id::text)::integer, 0);
